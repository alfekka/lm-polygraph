...
"ru": (
    "Используя факт, определите соответствующие слова "
    "в исходном предложении, которые помогают получить этот факт "
    "Пожалуйста, перечислите все слова через запятую, "
    "которые имеют отношение к данному факту "
    "в том порядке, в котором они появляются в исходном предложении. "
    "\nFact: {claim}\n"
    "Sentence: {sent}\nСлова из предложения, которые помогают "
    "получить факт, через запятую: "
)
                   }
